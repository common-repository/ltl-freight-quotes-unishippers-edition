...
if (!class_exists('group_unishippers_ltl_shipments')) {
    ...
    class group_unishippers_ltl_shipments extends Uinshipper_Ltl_Liftgate_As_Option
    {
            ...
            foreach ($package['contents'] as $item_id => $values) {
                ...
                if (!empty($origin_address)) {
                    ...
                    if (!$_product->is_virtual()) {
                        ...
                        $product_level_markup = $this->en_get_product_level_markup($_product, $variation_id, $product_id, $values['quantity']);
                        ...
                        if (!empty($product_tags)) {
                            ...
                            if (isset($unishippers_ltl_package[$locationId]['product_tags'])) {
                                $unishippers_ltl_package[$locationId]['product_tags'] = array_merge($unishippers_ltl_package[$locationId]['product_tags'], $product_tag_names);
                            } else {
                                $unishippers_ltl_package[$locationId]['product_tags'] = $product_tag_names;
                            }
                        } else {
                            $unishippers_ltl_package[$locationId]['product_tags'] = [];
                        }

                        // Product quantity
                        if (isset($unishippers_ltl_package[$locationId]['product_quantities'])) {
                            $unishippers_ltl_package[$locationId]['product_quantities'] += floatval($values['quantity']);
                        } else {
                            $unishippers_ltl_package[$locationId]['product_quantities'] = floatval($values['quantity']);
                        }

                        // Product price
                        if (isset($unishippers_ltl_package[$locationId]['product_prices'])) {
                            $unishippers_ltl_package[$locationId]['product_prices'] += (floatval($_product->get_price()) * floatval($values['quantity']));
                        } else {
                            $unishippers_ltl_package[$locationId]['product_prices'] = (floatval($_product->get_price()) * floatval($values['quantity']));
                        }
                    }
        ...
        */
        function en_get_product_level_markup($_product, $variation_id, $product_id, $quantity)
        {
