 ...
 */
if (!function_exists('en_unishippers_ltl_shipping_rules_template')) {
    function en_unishippers_ltl_shipping_rules_template($action = false)
    {
      ob_start();

      global $wpdb;
      $shipping_rules_list = $wpdb->get_results(
          "SELECT * FROM " . $wpdb->prefix . "eniture_unishippers_ltl_shipping_rules"
      );
              ...
              <?php
                if (count($shipping_rules_list) > 0) {
                  ...
                  foreach ($shipping_rules_list as $rule) {
                    ...
                    ?>
                      <tr id="sr_row_<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>" class="en_uni_ltl_sr_row">
                        ...
                        <td class="en_wd_warehouse_list_data">
                          <a href="#" class="en_unishippers_ltl_sr_status_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>" data-status="<?php echo $rule->is_active; ?>"><?php echo $rule->is_active ? 'Yes' : 'No'; ?></a>
                        </td>
                        <td class="en_wd_warehouse_list_data">
                          <!-- Edit rule link -->
                          <a href="#" class="en_unishippers_ltl_sr_edit_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>">
                            <img src="<?php echo plugins_url(); ?>/ltl-freight-quotes-unishippers-edition/warehouse-dropship/wild/assets/images/edit.png" title="Edit">
                          </a>
                          <!-- Delete rule link -->
                          <a href="#" class="en_unishippers_ltl_sr_delete_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>">
                            <img src="<?php echo plugins_url(); ?>/ltl-freight-quotes-unishippers-edition/warehouse-dropship/wild/assets/images/delete.png" title="Delete">
                          </a>
                        </td>
                      </tr>
                    <?php
                  ...
                  ?>
                    <tr class="new_warehouse_add en_uni_ltl_empty_row" data-id=0>
...
<div class="en_unishippers_ltl_shipping_rules_setting_section">
    <br />
    <!-- Add rule button -->
    <div class="en_sr_add_btn">
      <a href="#en_unishippers_ltl_add_sr_btn" title="Add Rule"
             class="en_unishippers_ltl_add_sr_btn button-primary" id="en_unishippers_ltl_add_shipping_rule_btn">Add Rule</a>
    </div>

    <div class="updated inline warehouse_deleted unishippers_ltl_sr_deleted">
      <p><strong>Success!</strong> Shipping rule is deleted successfully.</p>
    </div>
    <div class="updated inline warehouse_created unishippers_ltl_sr_created">
        <p><strong>Success!</strong> Shipping rule is added successfully.</p>
    </div>
    <div class="updated inline warehouse_updated unishippers_ltl_sr_updated">
        <p><strong>Success!</strong> Shipping rule is updated successfully.</p>
    </div>

    <!-- Shipping rules data table -->
    <?php en_unishippers_ltl_shipping_rules_template(); ?>

    <!-- Add popup for new rule -->
    <div id="en_unishippers_ltl_add_sr_btn" class="en_wd_warehouse_overlay">
        <div class="en_wd_add_warehouse_popup en_unishippers_ltl_add_sr_popup">
            <h2 class="warehouse_heading">Add Rule</h2>
            <a class="close" href="#">&times;</a>
            <div class="content" id="en_unishippers_ltl_sr_content" style="overflow-y: auto; height: 80vh;">

              <div class="already_exist unishippers_ltl_sr_already_exist">
                <strong>Error!</strong> Shipping rule with this name already exists.
              </div>
                <!-- Wordpress Form closed -->
                </form>

                <form id="unishippers_ltl_add_shipping_rule" role="form">
                    ...
                    <!-- Type -->
                    <div class="en_sr_form_control">
                        <label for="en_unishippers_ltl_sr_rule_type">Type</label>
                        <select name="en_unishippers_ltl_sr_rule_type" id="en_unishippers_ltl_sr_rule_type">
                    ...
                    <div class="en_sr_form_control en_liftgate_weight_restrictions">
                        <label for="en_sr_liftgate_weight_restrictions">If any item in the Cart exceeds this weight, do not offer an option for lift gate service if enabled. The maximum weight for liftgate service is 2000 lbs.</label>
                    ...
                    <!-- Form submit button -->
                    <div class="form-btns">
                        <input type="submit" name="en_wd_submit_warehouse" value="Save" class="save_warehouse_form en_unishippers_ltl_save_shipping_rule_form">
                    </div>
                </form>
            </div>
        </div>
