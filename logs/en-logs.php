...
if (!class_exists('EnUnishippresLfqLogs')) {
    class EnUnishippresLfqLogs
    {
        ...
        {
            $obj_classs = new \Unishippers_Curl_Request;
            $carrier_name = get_option('api_endpoint_unishippers_ltl') == 'unishippers_ltl_new_api' ? 'Unishippers LTL New API' : 'unishippersLtl';
            $data = array(
            ...
            $url = UNISHIPPERS_FREIGHT_DOMAIN_HITTING_URL . '/request-log/index.php';
            $logs = $obj_classs->unishippers_freight_get_curl_response($url, $data);
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $class_name = 'uni-ltl-log-' . $key . rand(1, 100);
