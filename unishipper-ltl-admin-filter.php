...
if (!function_exists('unishippers_ltl_hide_shipping_based_on_class')) {
    ...
    function unishippers_ltl_hide_shipping_based_on_class($available_methods)
    {
        ...
        foreach ($available_methods as $value) {
            ...
            if ($value->method_id == 'uni_ltl_shipping_method' || strpos($value->id, 'uni_ltl_shipping_method') !== false) {
                $plugin_rates_available = true;
                break;
            }
        }

        $plugin_rates = (empty(get_option('unishippers_ltl_backup_rates_display')) || get_option('unishippers_ltl_backup_rates_display') == 'no_plugin_rates') && $plugin_rates_available;
        $other_rates = get_option('unishippers_ltl_backup_rates_display') == 'no_other_rates' && count($available_methods) > 1;

        // Remove backup Rates in case plugin or other rates are available
        if (get_option('enable_backup_rates_unishippers_ltl') == 'yes' && ($plugin_rates || $other_rates)) {
            $backup_rate_id = 'uni_ltl_shipping_method:backup_rates';
        ...
        if (get_option('wc_settings_unishippers_freight_allow_other_plugins') == 'no'
...
if (!function_exists('unishippers_ltl_save_unishippers_carrier_status')) {
    ...
    function unishippers_ltl_save_unishippers_carrier_status($post_id)
    {
        ...
        if (isset($action) && $action == 'save_unishippers_carrier_status') {
            ...
            $unishippers_ltl_freight_get = $wpdb->get_results("SELECT * FROM $table_name order by unishippers_carrierName ASC");
            ...
            foreach ($unishippers_ltl_freight_get as $unishippers_ltl_freight_get_value):
                ...
                if (isset($_POST[$unishippers_ltl_freight_get_value->unishippers_carrierSCAC . $unishippers_ltl_freight_get_value->id]) && $_POST[$unishippers_ltl_freight_get_value->unishippers_carrierSCAC . $unishippers_ltl_freight_get_value->id] == 'on') {
                    ...
                    $wpdb->query($wpdb->prepare("UPDATE $table_name SET carrier_status = '%s' WHERE unishippers_carrierSCAC = '$unishippers_ltl_freight_get_value->unishippers_carrierSCAC'", '1'));
                } else {

                    $wpdb->query($wpdb->prepare("UPDATE $table_name SET carrier_status = '%s' WHERE unishippers_carrierSCAC = '$unishippers_ltl_freight_get_value->unishippers_carrierSCAC' ", '0'));
                }
            endforeach;
        }
    }
