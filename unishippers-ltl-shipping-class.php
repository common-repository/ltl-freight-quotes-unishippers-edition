        ...
        if (!class_exists('WC_unishippers_Shipping_Method')) {
            ...
            class WC_unishippers_Shipping_Method extends WC_Shipping_Method
            {
                ...
                public function calculate_shipping($package = [], $eniture_admin_order_action = false)
                {
                    ...
                    $shipping_rule_obj = new EnUnishippersLtlShippingRulesAjaxReq();
                    $shipping_rules_applied = $shipping_rule_obj->apply_shipping_rules($unishippers_ltl_package);
                    ...
                    if (!empty($unishippers_ltl_package)) {
                        ...
                        foreach ($unishippers_ltl_package as $key => $sPackage) {
                            if (array_key_exists('unishippers_ltl', $sPackage)) {
                                ...
                                $response = $unishippers_ltl_res_inst->unishippers_ltl_shipping_get_web_quotes($web_service_arr, $unishippers_ltl_package, $key);

                                // Backup rates
                                if ((empty($response) && get_option('unishippers_ltl_backup_rates_carrier_returns_error') == 'yes') || (isset($response['error']) && $response['error'] == 'backup_rate' && get_option('unishippers_ltl_backup_rates_carrier_fails_to_return_response') == 'yes')) {
                                    $this->uni_ltl_backup_rates();
                                    $response = [];
                                }
                    ...
                    if ((count($quotes) > 1 || $smpkgCost > 0) || $no_param_multi_ship == 1 || !empty($virtual_rate)) {
                        ...
                        foreach ($quotes as $key => $quote) {
                            ...
                            if (isset($quote['excluded'])) unset($quote['excluded']);
                            ...
                            if (isset($this->quote_settings['liftgate_delivery_option']) &&
                                ...
                                $this->en_fdo_meta_data['UNISHIPPERS_NOTLIFT'][$key]['rate']['cost'] = $this->add_handling_fee($s_cost, $handling_fee);
                            }

                            $multi_cost += $this->add_handling_fee($_cost, $handling_fee);
                            $this->en_fdo_meta_data['UNISHIPPERS_LIFT'][$key]['rate']['cost'] = $this->add_handling_fee($_cost, $handling_fee);
                        }

                        // Excluded accessorials
                        $en_accessorial_excluded = apply_filters('en_unishippers_ltl_accessorial_excluded', []);
                        ...
                        $quote = reset($quotes);
                        $excluded = isset($quote['excluded']) ? $quote['excluded'] : false;
                        $quote = $this->remove_array($quote, 'excluded');
                        $simple_quotes = (isset($quote['simple_quotes'])) ? $quote['simple_quotes'] : array();
                        $quote = $this->remove_array($quote, 'simple_quotes');
                ...
                {
                    if (!is_array($quote) || !isset($quote[$remove_index])) return $quote;
                ...
                function unishippers_ltl_shipping_quote_settings()
                {
                    ...
                    $enable_carriers = $wpdb->get_results("SELECT `unishippers_carrierSCAC` FROM $table_name Where carrier_status ='1'");
                    ...
                    $this->unishippers_ltl_res_inst->quote_settings['handling_weight'] = get_option('handling_weight_unishippers_ltl');
                    $this->unishippers_ltl_res_inst->quote_settings['maximum_handling_weight'] = get_option('maximum_handling_weight_unishippers_ltl');
                }
                ...
                * @return void
                * */
                function uni_ltl_backup_rates()
                {
                    if (get_option('enable_backup_rates_unishippers_ltl') != 'yes' || (get_option('unishippers_ltl_backup_rates_carrier_fails_to_return_response') != 'yes' && get_option('unishippers_ltl_backup_rates_carrier_returns_error') != 'yes')) return;

                    $backup_rates_type = get_option('unishippers_ltl_backup_rates_category');
                    $backup_rates_cost = 0;

                    if ($backup_rates_type == 'fixed_rate' && !empty(get_option('uni_ltl_backup_rates_fixed_rate'))) {
                        $backup_rates_cost = get_option('uni_ltl_backup_rates_fixed_rate');
                    } elseif ($backup_rates_type == 'percentage_of_cart_price' && !empty(get_option('uni_ltl_backup_rates_cart_price_percentage'))) {
                        $cart_price_percentage = floatval(str_replace("%", "", get_option('uni_ltl_backup_rates_cart_price_percentage')));
                        $backup_rates_cost = ($cart_price_percentage * WC()->cart->get_subtotal()) / 100;
                    } elseif ($backup_rates_type == 'function_of_weight' && !empty(get_option('uni_ltl_backup_rates_weight_function'))) {
                        $cart_weight = wc_get_weight(WC()->cart->get_cart_contents_weight(), 'lbs');
                        $backup_rates_cost = get_option('uni_ltl_backup_rates_weight_function') * $cart_weight;
                    }

                    if ($backup_rates_cost > 0) {
                        $backup_rates = array(
                            'id' => $this->id . ':' . 'backup_rates',
                            'label' => get_option('unishippers_ltl_backup_rates_label'),
