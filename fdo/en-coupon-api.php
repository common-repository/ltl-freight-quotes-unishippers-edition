...
if (!class_exists('EnUnishippersLtlCouponAPI')) {
    class EnUnishippersLtlCouponAPI
    {
        public function __construct()
        {
            ...
            add_action('wp_ajax_nopriv_unishippers_fd', [$this, 'unishippers_fd_api']);
            add_action('wp_ajax_unishippers_fd', [$this, 'unishippers_fd_api']);
            add_action('rest_api_init', [$this, 'en_rest_api_init_status_unishippers']);
        }
         ...
         */
        function unishippers_fd_api()
        {
            $store_name = unishippers_freight_get_domain();
            $company_id = $_POST['company_id'];
            $data = [
                'plateform'  => 'wp',
                'store_name' => $store_name,
                'company_id' => $company_id,
                'fd_section' => 'tab=unishippers_freight&section=section-5',
            ];
         ...
         */
        function en_rest_api_init_status_unishippers()
        {
            register_rest_route('fdo-company-id', '/update-status', array(
                'methods' => 'POST',
                'callback' => [$this, 'en_unishippers_fdo_data_status'],
                'permission_callback' => '__return_true'
            ));
        }
         ...
         * @return array|void
         */
        function en_unishippers_fdo_data_status(WP_REST_Request $request)
        {
